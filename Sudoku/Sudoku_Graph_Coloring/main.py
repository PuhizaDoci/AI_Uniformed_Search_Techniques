...
class SudokuBoard: 
    def __init__(self): 
        ...
        self.mappedGrid = self.getMappedMatrix() # Maps all the ids to the position in the matrix

    def getMappedMatrix(self): 
    ...
    def solveGraphColoring(self, m =9): 
        ...
        if self.graphColorUtility(m =m, color=color, v =1, given=given) is None:
            print(":(")
            return False
        count = 1
        for row in range(9): 
    ...
    def graphColorUtility(self, m, color, v, given):
        if v == self.sudokuGraph.graph.totalV+1: 
            return True

        for c in range(1, m+1): 
            if self.isSafeToColor(v, color, c, given) == True:
                color[v] = c
                if self.graphColorUtility(m, color, v+1, given): 
                    return True
            if v not in given: 
                color[v] = 0

    def isSafeToColor(self, v, color, c, given): 
